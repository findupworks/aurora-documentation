...
Aurora Workflow.proporciona un registro completo para todas las ejecuciones de flujos de trabajo, dándote visibilidad total sobre cómo se ejecutan tus flujos de trabajo, qué datos fluyen a través de ellos y dónde podrían ocurrir problemas.

## Sistema de registro

Aurora Workflow.ofrece dos interfaces de registro complementarias para adaptarse a diferentes flujos de trabajo y casos de uso:
...
<Tabs items={["Output", "Input"]}>
  <Tab>
    **Pestaña de salida** muestra el resultado de la ejecución del bloque: -
    Datos estructurados con formato JSON - Renderizado de Markdown para
    contenido generado por IA - Botón de copiar para fácil extracción de datos
  </Tab>

  <Tab>
    **Pestaña de entrada** muestra lo que se pasó al bloque: - Valores de
    variables resueltos - Salidas referenciadas de otros bloques - Variables de
    entorno utilizadas - Las claves API se redactan automáticamente por
    seguridad
  </Tab>
</Tabs>
...
<Callout type="info">
  Las instantáneas de flujo de trabajo solo están disponibles para ejecuciones
  posteriores a la introducción del sistema mejorado de registro. Los registros
  migrados más antiguos muestran un mensaje de "Estado registrado no
  encontrado".
</Callout>
...
- Aprende sobre [Cálculo de costos](/workflow/pt-BR/execution/costs) para entender los precios de los flujos de trabajo
- Explora la [API externa](/workflow/pt-BR/execution/api) para acceso programático a los registros
- Configura [notificaciones por Webhook](/workflow/pt-BR/execution/api#webhook-subscriptions) para alertas en tiempo real
