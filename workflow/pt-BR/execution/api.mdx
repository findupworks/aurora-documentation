...
Aurora Workflow.proporciona una API externa completa para consultar registros de ejecución de flujos de trabajo y configurar webhooks para notificaciones en tiempo real cuando los flujos de trabajo se completan.
...
curl -H "x-api-key: YOUR_API_KEY" \
  https://workflow.aurora-ai.co/api/v1/logs?workspaceId=YOUR_WORKSPACE_ID
```

Puedes generar claves de API desde la configuración de usuario en el panel de control de Aurora Workflow.
...
<Tabs items={['Request', 'Response']}>
  <Tab value="Request">
    ...
    ```http
    GET /api/v1/logs/workflow/pt-BR/executions/{executionId}
    ```
...
Recibe notificaciones en tiempo real cuando se completan las ejecuciones de flujos de trabajo. Los webhooks se configuran a través de la interfaz de usuario de Aurora Workflow.en el editor de flujos de trabajo.
...
Cuando se completa la ejecución de un flujo de trabajo, Aurora Workflow.envía una solicitud POST a tu URL de webhook:
...
```json
{
  ...
  "links": {
    ...
    "execution": "/v1/logs/workflow/pt-BR/executions/exec_def456"
  }
}
```
...
2. **Seguridad de webhook**: Siempre configura un secreto de webhook y verifica las firmas para asegurar que las solicitudes provienen de Aurora Workflow.
...
async function pollLogs() {
  ...
  const response = await fetch(
    `https://workflow.aurora-ai.co/api/v1/logs?${params}`,
    {
      headers: {
